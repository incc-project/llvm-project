...
DebugArangeSection::DebugArangeSection(const llvm::object::ELF64LE::Shdr *shdr,
  ...
  while (start < end) {
    ...
    size_t header_size =
        4 + 2 + 4 + 1 +
        1; // unit_length+version+debug_info_offset+addr_size+seg_size
    size_t padding =
        (2 * set.address_size) - (header_size % (2 * set.address_size));
...
void DebugArangeSection::writeDataTo(char *buffer) {
...
void DebugArangeSection::dumpData(std::ostream &oss) const {
  ...
  for (size_t i = 0; i < aranges.size(); i++) {
    ...
    oss << ", version = 0x" << DebugConvert::intToHex(set.version, 4);
    oss << ", cu_offset = 0x"
        << DebugConvert::intToHex(set.debug_info_offset, 8);
    oss << ", addr_size = 0x" << DebugConvert::intToHex(set.address_size, 2)
        << ", seg_size = 0x" << DebugConvert::intToHex(set.segment_size, 2)
        << "\n";
    for (const auto &r : set.ranges) {
      oss << "    [0x"
          << DebugConvert::intToHex(r.address, set.address_size * 2) << ", 0x"
          << DebugConvert::intToHex(r.address + r.length, set.address_size * 2)
          << ")\n";
    }
  }
}
