...
void DebugStrOffsetsSection::applyRelocations(
  ...
  for (const auto &rel : relocs) {
    ...
    for (size_t i = 0; i < tables.size(); ++i) {
      ...
      if (offsetInTable % 4 != 0)
        continue;

      size_t entryIndex = offsetInTable / 4;
      if (entryIndex >= allOffsets[i].size())
        continue;
...
void DebugStrOffsetsSection::dumpData(std::ostream &oss) const {
  ...
  for (size_t i = 0; i < tables.size(); ++i) {
    ...
    oss << "0x" << std::hex << std::setw(8) << std::setfill('0')
        << offsetBases[i] << ": Contribution size = " << std::dec << sizes[i]
        << ", Format = DWARF32"
...
int DebugStrOffsetsSection::getTableIndex(uint64_t baseOffset) const {
...
uint32_t DebugStrOffsetsSection::getStringOffset(int tableIndex,
                                                 uint32_t strxIndex) const {
...
std::string
DebugStrOffsetsSection::getStringFromStrx(int tableIndex,
                                          uint32_t strxIndex) const {
