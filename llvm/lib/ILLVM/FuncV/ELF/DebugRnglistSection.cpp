...
void DebugRnglistSection::dumpData(std::ostream &oss) const {
  oss << ".debug_rnglists contents:\n";
  oss << "range list header: "
      << "length = 0x" << std::hex << std::setw(8) << std::setfill('0')
      << header.unit_length << ", format = DWARF32"
      << ", version = 0x" << std::setw(4) << header.version
      << ", addr_size = 0x" << std::setw(2)
      << static_cast<int>(header.addr_size) << ", seg_size = 0x" << std::setw(2)
      << static_cast<int>(header.seg_size) << ", offset_entry_count = 0x"
      << std::setw(8) << header.offset_entry_count << "\n";
  ...
  for (auto &rangeList : rangeLists) {
    ...
    for (const auto &entry : rangeList.second) {
      ...
      case 0x07: {
        oss << "[0x" << std::hex << std::setw(16) << std::setfill('0')
            << baseOffset + entry.value0 << ", 0x" << std::setw(16)
            << baseOffset + entry.value1 << ")\n";
        break;
      }
      default:;
      }
...
void DebugRnglistSection::registerAddrBase(uint32_t index,
                                           uint64_t addrBaseOffset) const {
