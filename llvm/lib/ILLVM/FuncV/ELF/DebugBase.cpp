...
std::string DebugTemp::skipFormValue(uint64_t form, const uint8_t *&p,
                                     const uint8_t *end, uint64_t dieOffset) {
  switch (form) {
  case 0x01: { // DW_FORM_addr
    uint64_t val = *reinterpret_cast<const uint64_t *>(p);
    p += 8;
    return "addr: 0x" + DebugConvert::intToHex(val, 16);
  }
  case 0x03: { // DW_FORM_block2
    ...
    for (uint16_t i = 0; i < len && p < end; ++i)
      oss << DebugConvert::intToHex(*p++, 2) << (i + 1 < len ? " " : "");
    oss << "]";
    return oss.str();
  }
  ...
  case 0x05: {
    ...
    p += 2;
    return "0x" + DebugConvert::intToHex(val, 4);
  }
  case 0x06: {
    uint32_t val = *reinterpret_cast<const uint32_t *>(p);
    p += 4;
    return "0x" + DebugConvert::intToHex(val, 8);
  }
  case 0x07: {
    uint64_t val = *reinterpret_cast<const uint64_t *>(p);
    p += 8;
    return "0x" + DebugConvert::intToHex(val, 16);
  }
  ...
  case 0x0b: {
    uint8_t val = *p++;
    return "0x" + DebugConvert::intToHex(val, 2);
  }
  ...
  case 0x0d: { // DW_FORM_sdata
    unsigned size = 0;
    int64_t val = DebugConvert::decodeSLEB128(p, &size, end);
    p += size;
  ...
  case 0x0f: { // DW_FORM_udata
    unsigned size = 0;
    uint64_t val = DebugConvert::decodeULEB128(p, &size, end);
    p += size;
    return std::to_string(val);
  }
  ...
  case 0x20: { // DW_FORM_ref_sig8
    ...
    p += 8;
    return "ref: 0x" + DebugConvert::intToHex(ref, 16);
  }
  case 0x11: {
    uint8_t ref = *p++;
    return "ref: 0x" + DebugConvert::intToHex(ref, 2);
  }
  case 0x12: {
    uint16_t ref = *reinterpret_cast<const uint16_t *>(p);
    p += 2;
    return "ref: 0x" + DebugConvert::intToHex(ref, 4);
  }
  case 0x13: {
    uint32_t ref = *reinterpret_cast<const uint32_t *>(p);
    p += 4;
    return "ref: 0x" + DebugConvert::intToHex(ref, 8);
  }
  case 0x14: {
    uint64_t ref = *reinterpret_cast<const uint64_t *>(p);
    p += 8;
  ...
  case 0x15: {
    unsigned len = 0;
    uint64_t ref = DebugConvert::decodeULEB128(p, &len, end);
    p += len;
    return "ref: 0x" + DebugConvert::intToHex(ref, 8);
  }
  ...
  case 0x17: {
    uint32_t val = *reinterpret_cast<const uint32_t *>(p);
    p += 4;
    return "sec_offset: 0x" + DebugConvert::intToHex(val, 8);
  }
  case 0x18: {
    unsigned size = 0;
    uint64_t exprLen = DebugConvert::decodeULEB128(p, &size, end);
    p += size;
    std::ostringstream expr;
    expr << "exprloc[";
    for (uint64_t i = 0; i < exprLen && p < end; ++i)
      expr << DebugConvert::intToHex(*p++, 2) << (i + 1 < exprLen ? " " : "");
    expr << "]";
    return expr.str();
  }
  case 0x19: {
  ...
  case 0x1b: {
    unsigned len = 0;
    uint64_t index = DebugConvert::decodeULEB128(p, &len, end);
    p += len;
    return "addrx[]" + DebugConvert::intToHex(index, 2);
  }
  case 0x1d: { // DW_FORM_strp_sup
    uint32_t offset = *reinterpret_cast<const uint32_t *>(p);
    p += 4;
    // return "\"" + debugStrSup.getString(offset) + "\"";  // 若有
    // .debug_str_sup 数据
    return "strp_sup[0x" + DebugConvert::intToHex(offset, 8) + "]";
  }
  case 0x1e: { // DW_FORM_data16
    std::ostringstream out;
    out << "data16[";
    for (int i = 0; i < 16; ++i)
      out << DebugConvert::intToHex(p[i], 2) << (i < 15 ? " " : "");
    out << "]";
    p += 16;
    return out.str();
  }
  case 0x1f: { // DW_FORM_line_strp
    uint32_t offset = *reinterpret_cast<const uint32_t *>(p);
    p += 4;
    // return "\"" + lineStrSection.getString(offset) + "\"";  // 若有
    // .debug_line_str 支持
    return "line_strp[0x" + DebugConvert::intToHex(offset, 8) + "]";
  }
  ...
  case 0x25: { // DW_FORM_strx1
    ...
    //    }
    return DebugConvert::intToHex(index, 2);
  }
  ...
  default:
    return "(unhandled form: 0x" + DebugConvert::intToHex(form, 2) + ")";
  }
}

void FormValueRaw::writeFormValue(const FormValueRaw &val,
                                  std::vector<uint8_t> &out) {
  switch (val.form) {
  case 0x01:
  case 0x07:
  case 0x14:
  case 0x20:
  case 0x24: { // addr, data8, ref8
    for (int i = 0; i < 8; ++i)
      out.push_back((val.value >> (i * 8)) & 0xFF);
    break;
  }
  case 0x05:
  case 0x12: { // data2, ref2
  ...
  case 0x0d: { // sdata
    DebugConvert::encodeSLEB128(val.value, out);
    break;
  }
  case 0x0f:
  case 0x15:
  ...
  case 0x23:
  case 0x1b: { // udata
    DebugConvert::encodeULEB128(val.value, out);
    break;
  }
  case 0x08: { // string
  ...
  case 0x09:
  case 0x18: { // block / exprloc
    DebugConvert::encodeULEB128(val.blockData.size(), out);
    out.insert(out.end(), val.blockData.begin(), val.blockData.end());
    break;
  }
  case 0x0a: { // block1
  ...
  default:
    llvm::errs() << "Unsupported form in writeFormValue: 0x"
                 << DebugConvert::intToHex(val.form, 2) << "\n";
  }
}
