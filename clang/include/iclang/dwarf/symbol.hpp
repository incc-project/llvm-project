...
protected:
    ...
    // For section type, we will set name to the actual string, and set st_name to 0.
    uint32_t st_name;
    ...
    // i.e., shndx == SHN_UNDEF || (SHN_LORESERVE <= shndx && shndx < SHN_XINDEX)
    // otherwise, we use secIdx to represent the target section index.
    // Note that we do not consider SHN_XINDEX as a special shndx.
    // SHN_XINDEX means that the actual section header index is too large to fit in this field.
...
public:
    Symbol(const uint32_t _st_name, const std::shared_ptr<StrRef> &_name,
           const uint64_t _st_value, const uint32_t _st_size,
           const unsigned char _st_info, const unsigned char _st_other,
           const uint64_t _st_shndx, const int _specialShndx,
           const std::shared_ptr<IdxRef> &_secIdx,
           const std::shared_ptr<IdxRef> &_idx)
        : st_name(_st_name), name(_name), st_value(_st_value),
    ...
    Symbol(const llvm::object::ELF64LE::Sym *elf_sym)
        : Symbol(elf_sym->st_name, nullptr, elf_sym->st_value, elf_sym->st_size,
                 elf_sym->st_info, elf_sym->st_other, elf_sym->st_shndx, -1,
                 nullptr, nullptr) {}
    ...
    void setSpecialShndx(const int _specialShndx) { specialShndx = _specialShndx; }
    ...
    void writeDataTo(llvm::object::ELF64LE::Sym *sym) {
        sym->st_name = st_name;
